...
LIST_URLS = [
    f"https://www2.assemblee-nationale.fr/sycomore/liste/{i}"
...
def scrape_depute_fiche(url):
    try:
        ...
        # 3. Mandates
        mandates = []
        # Sycomore usually uses <dl> for mandate details
...
# Phase 1: Collect all links from the 14 list pages
...
for list_url in LIST_URLS:
    ...
    try:
        ...
        soup = BeautifulSoup(r.content, "html.parser")
        # Links are usually like /sycomore/fiche/(id)
...
with open('deputes_sycomore.csv', 'w', newline='', encoding='utf-8') as f:
    fieldnames = ["nom", "prenom", "role", "from_date", "to_date", "birth_date", "death_date", "sycomore_id", "url"]
    writer = csv.DictWriter(f, fieldnames=fieldnames)
    writer.writeheader()

    for i, link in enumerate(sorted(list(all_deputes_links))):
        print(f"[{i + 1}/{len(all_deputes_links)}] Scaping: {link}")

        results = scrape_depute_fiche(link)
        for row in results:
            writer.writerow(row)

        # Pause to be polite to server
        time.sleep(0.5)

print("Scrape complete. Results saved in 'deputes.csv'")